...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (lastSubmissionTime) {
    const remainingTime = COOLDOWN_TIME - (Date.now() - parseInt(lastSubmissionTime));
    if (remainingTime > 0) {
      disableQuizSubmission(remainingTime);
    }
  }
  ...
  form.addEventListener("submit", (e) => {
    ...
    disableQuizSubmission(COOLDOWN_TIME);

    alert("Quiz submitted successfully!");
    form.reset();
  });

  function disableQuizSubmission(remainingTime) {
    const cooldownTimerDisplay = document.createElement("div");
    cooldownTimerDisplay.id = "cooldownTimer";
    cooldownTimerDisplay.textContent = `You can submit another quiz in: ${formatTime(remainingTime)}`;
    form.parentElement.insertBefore(cooldownTimerDisplay, form);
  ...
  function startCooldownTimer(remainingTime) {
    const cooldownTimer = setInterval(() => {
      ...
      if (remainingTime <= 0) {
        ...
        location.reload(); // Reload the page to allow quiz submission
      } else {
        document.getElementById("cooldownTimer").textContent = `You can submit another quiz in: ${formatTime(remainingTime)}`;
      }
    }, 1000);
  }
